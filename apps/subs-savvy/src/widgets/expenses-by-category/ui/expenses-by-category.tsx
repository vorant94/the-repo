...
export const ExpensesByCategory: FC<ExpensesByCategoryProps> = ({
  className,
}) => {
  const { t } = useTranslation();

  const periodOptions: Array<SegmentedControlItem> = [
  ...
  const aggregatedSubscriptionsForYear = aggregateSubscriptionsByCategory(
    subscriptions,
    (subscription) => calculateSubscriptionPriceForYear(subscription),
  );
  const aggregatedSubscriptionsFor6Months = aggregateSubscriptionsByCategory(
    subscriptions,
    (subscription) =>
  ...
  >;
  const subsToShow = periodToAggregatedSubs[period];

  const [activeIndex, setActiveIndex] = useState<number>(-1);
  const updateActiveIndex = (_: unknown, index: number) => {
