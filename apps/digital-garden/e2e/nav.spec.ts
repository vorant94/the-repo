...
test("has desktop navigation links", { tag: ["@desktop"] }, async ({
  page,
}) => {
  await page.goto("/");
...
test("has mobile nav modal instead of desktop navigation links", {
  tag: ["@mobile"],
}, async ({ page }) => {
  await page.goto("/");
...
test("closes mobile nav on screen resize", { tag: ["@mobile"] }, async ({
  page,
}) => {
  await page.goto("/");
