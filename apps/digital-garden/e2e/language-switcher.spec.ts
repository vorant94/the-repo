...
test("redirect from root to default lang", { tag: ["@desktop"] }, async ({
  page,
}) => {
  await page.goto("/");
...
test("swap language and stay at the same page if its not post page", {
  tag: ["@desktop"],
}, async ({ page }) => {
...
test("swap language and stay at the same page if its post page that has translation", {
  tag: ["@desktop"],
}, async ({ page }) => {
...
test("swap language and redirect to home if its post page that has no translation", {
  tag: ["@desktop"],
}, async ({ page }) => {
